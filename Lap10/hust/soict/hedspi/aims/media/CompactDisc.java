...
public class CompactDisc extends Disc{
    ...
    public void addTrack(Track track){
        int fg = 0;
        for(Track e : tracks){
            if(e.getTitle().equals(track.getTitle()) && e.getLength() == track.getLength()){
                fg = 1;
                break;
            }
        }
    ...
    public int compareTo(Media e) {
        if(e instanceof CompactDisc){
            if(this.getNumOfTrack() > ((CompactDisc) e).getNumOfTrack())
                return 1;
            else if (this.getNumOfTrack() < ((CompactDisc) e).getNumOfTrack())
                return -1;
            else{
                if(this.getLength() > ((CompactDisc) e).getLength())
                    return 1;
                else if (this.getLength() > ((CompactDisc) e).getLength())
                    return -1;
                else
